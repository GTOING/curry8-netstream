...
在设置区的控制模式、真实协议档案、触发策略、Curry 端口或回放块号上滚动鼠标滚轮/触控板，只会滚动设置面板，不会改动当前选项或数值；错误区和只读诊断内容仍可独立滚动。下拉框关闭时，点击后选项或用键盘方向键选择；打开列表后可以滚轮浏览，但要点击条目或用键盘确认才会提交。端口等数值通过点击后输入、微调箭头或键盘方向键调整；回放块号也可用“上一块/下一块”按钮导航。
...
连接前在“会话记录与离线回放”中选择保存目录并启用会话记录；只有记录启用后才能勾选“同时自动生成分期 CSV”。CSV 会写入本次新建的 `session_<id>\stage_labels.csv`，每个处理结果完成后立即追加一行，UTF-8 BOM 可供 Windows Excel 识别。实时连接完全结束后，可从回放入口选择“导出/重新生成 CSV”；导出重建同目录文件，不修改 JSONL/NPY 权威归档，只包含可验证前缀。若 CSV 正被 Excel 打开，请先关闭后再重建。旧会话若没有本机时间字段，会保留原 UTC、将本机时间留空并标为不可用。

表中的本机时间表示控制应用收到最后贡献网络包时记录的本地时区时间，不能用于 Curry/Rally 时钟同步、EEG 精确采集时刻或物理刺激输出时序。创建 CSV 失败会阻止本次网络采集开始；采集中追加失败会在界面标记 CSV 不完整，但继续保留权威记录和必要停止控制。完整字段说明与故障恢复见[会话记录与分期 CSV 指南](SESSION_RECORDING_GUIDE.md)。
