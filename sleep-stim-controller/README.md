> GitHub 发布快照：本目录包含控制器及其当前 `curry8-netstream/` 依赖源码（含本地适配），克隆后进入本目录执行 `uv sync --locked`。无需再次克隆 Curry 或应用补丁。下文新建本地工作区的克隆/补丁步骤仅供缺失依赖时参考。原参考材料不随本次上传，历史报告内的本机绝对路径仅在原开发机器有效。正式目标平台为 Windows，模型暂缓，Windows 实测尚未完成。
...
Curry 以本地可编辑依赖安装，源码改动可直接生效。若在其他机器重建目录，需先准备同级的上游仓库：

```sh
git clone https://github.com/GTOING/curry8-netstream.git curry8-netstream
git -C curry8-netstream checkout 0d1067c5ea557dac5f15cdd92268e21b6936f83b
...
以上重建命令针对新克隆使用，不用于覆盖已有工作区。`uv.lock` 固定 Python 包版本，本地路径依赖的源码版本以这里记录的 Git 提交为准。
...
sleep/
  ...
  curry8-netstream/             独立 Git 仓库，Curry 通信本地依赖
  docs/                        需求与后续接口合同
...
根级 `tests/` 包含控制器、分期/会话格式、离屏 UI 和本机回环合成 Curry 服务测试；`curry8-netstream/` 仍是独立上游仓库，不复制其源码或测试到根包，也不将参考 Demo 当作产品代码。
...
当前根目录尚未初始化 Git；`curry8-netstream/` 保留自身历史和远端。P1 对 Curry 客户端只增加可取消接收、状态/会话/错误结果与关闭清理能力，保持原有 CLI 和线协议布局。根级 `.gitignore` 排除该独立克隆及本地参考原件，但不删除文件；没有创建主仓库、提交或推送。
