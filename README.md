...
P-GUI 已移除实时和回放波形，EEG 请在 Curry 8 查看。控制台保留完整 EEG 接收、校验、可选 ONNX 睡眠分期、保存和逐块只读回放。Curry 网络包可以是任意合法短包，控制器按连续采样点组装不重叠的 30 秒分析窗口；包频率不再决定分析窗口。Windows 锁定安装、原生 GUI、中文/空格路径权限和 TCP/UDP 合成回环已验证通过；真实 Curry/Rally 设备仍未联调。
