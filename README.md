...
P-GUI 已移除实时和回放波形，EEG 请在 Curry 8 查看。控制台保留完整 EEG 接收、校验、ONNX 睡眠分期、保存和逐块只读回放。Windows 锁定安装、原生 GUI、中文/空格路径权限和 TCP/UDP 合成回环已验证通过；真实 Curry/Rally 设备仍未联调。

当前已完成 P1/P2 桌面、Curry 脑电接入、可选会话记录和只读离线回放，并实现 P3 睡眠期策略、协议适配及受控本机 UDP 模拟联调。默认发布包已接入单通道 LiteSleepNet EDF20 FP32 ONNX 模型；真实电刺激、真实 Rally 设备及跨设备时钟同步仍未接入。
...
窗口默认使用 `127.0.0.1:4455`，连接期间地址和端口不可修改。默认窗口 1000×720，最小 800×600；顶部固定连接、模式和错误区域，主体依次展示数据/分期摘要、模型配置、刺激配置、会话记录/回放。连接前可选择 ONNX 文件并填写一个与 Curry 握手标签精确匹配的通道名，握手完成后界面显示可用通道。摘要显示实际会话、块和样本区间、通道、采样率；长错误、方案和路径可滚动阅读与复制。

模型输入固定解释为 µV，并连续应用 50 Hz 工频处理和 0.3–35 Hz 带通，再重采样到 100 Hz；不做逐样本 z-score 或其他静默幅值归一化。模型时间维度从 ONNX 输入自动读取，必须是 3000 的正整数倍：3000 点为当前 30 秒，6000 点为前一段加当前段，9000 点依此类推；会话开头缺少历史时复制最早可用 epoch。100 Hz 数据的 50 Hz 位于 Nyquist 点，无法单独设计数字陷波，此时由 35 Hz 低通边界排除；源采样率高于 100 Hz 时显式应用 50 Hz notch。详细合同见 [ONNX 睡眠分期合同](docs/P_MODEL_ONNX_CONTRACT.md)。
