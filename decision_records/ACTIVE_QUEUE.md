# 当前任务队列

更新：2026-09-25。当前角色：决策主线程。

## 当前目标与状态

2026-09-25 当前权威状态：P4-B 已通过 PR #4 合并；Issue 5 与 Issue 6 工程范围通过验收并发布于 `main@cf88643`；Issue 7 工程范围通过验收并发布于 `main@98c4100`；Issue 8 工程范围通过验收并发布于 `main@72bd2ef`。远端 Issue #5–#8 均以 completed 关闭。现代化 GUI 已在 `feature/modern-control-ui` 完成收口并通过主线程工程验收；尚未提交推送或合并，Windows 原生界面仍待验证。以下早期“未提交/待实现”等叙述是当时的历史状态。

- [现代化 GUI 调研与设计](../docs/P_GUI_MODERNIZATION_DESIGN.md)、[实施计划](plans/P_GUI_MODERNIZATION_PLAN.md)及[执行任务](../docs/tasks/P_GUI_MODERNIZATION_EXECUTION.md)已在 `feature/modern-control-ui` 完成实现与[收口任务](../docs/tasks/P_GUI_MODERNIZATION_CLOSEOUT.md)；[主线程验收记录末节](../reports/P_GUI_MODERNIZATION_ACCEPTANCE.md)判定工程范围通过，首次暂不通过为历史。执行者最终定向 61 passed、完整回归 254 passed；主线程仅复现两项关键状态，未重跑全量。改动未提交、推送或合并；Windows 原生鼠标／缩放／高对比度及真机未验证。

- 远端 [Issue #5](https://github.com/GTOING/curry8-netstream/issues/5) 与 [Issue #6](https://github.com/GTOING/curry8-netstream/issues/6) 已于 2026-09-24 以 completed 关闭；获取时均无评论，原始状态归档见 [Issue 快照](../reports/remote_issues/2026-09-22/issue-5.md) 及 [#6](../reports/remote_issues/2026-09-22/issue-6.md)。
- 远端 [Issue #7：会话分期 CSV 自动续写与本机时间](https://github.com/GTOING/curry8-netstream/issues/7) 已于 2026-09-24 以 completed 关闭；两项收口通过[主线程工程验收](../reports/ISSUE7_SESSION_CSV_ACCEPTANCE.md)，实现与证据已提交推送。执行者最终定向 5 passed、完整回归 246 passed；主线程未重跑全量。Windows/Excel 与真实设备验证未执行；CSV 是会话 JSONL/NPY 的可选派生表，本机接收时间不代表 EEG 精确采集时刻。
- 远端 [Issue #8：GUI 滚轮防误触](https://github.com/GTOING/curry8-netstream/issues/8) 已于 2026-09-24 以 completed 关闭；[限界执行任务](../docs/tasks/ISSUE8_GUI_WHEEL_GUARD_EXECUTION.md)已完成并通过[主线程工程验收](../reports/ISSUE8_GUI_WHEEL_GUARD_ACCEPTANCE.md)，实现与证据已提交推送至 `main@72bd2ef`。执行者 UI 10 passed、完整回归 248 passed；主线程未重跑全量。Windows 原生滚轮/触控板行为仍待验证。
- Issue 5 工程范围收口验收通过（2026-09-23）：启用零基线 Stop、实际 Start 发送责任、取消锁顺序及重新启用旧结果边界已复核。执行者最终完整回归 210 passed，主线程未重跑；详见 [验收记录末节](../reports/ISSUE5_RALLY_ARMING_ACCEPTANCE.md) 与 [执行报告](../reports/ISSUE5_RALLY_ARMING_FIX_REPORT.md)。真实 Rally/Windows 物理验收仍待完成。
- Issue 6 工程范围收口验收通过（2026-09-23）：一般未发送 Start/Apply 不再自动重排，只有新合格期别明确取代旧 Apply 才在资格复核后规划最新协议；容量、发送异常与 SD 到期均有限收尾。执行者最终定向 38 passed、完整 237 passed；主线程按代码与关键测试复核，未重跑全量。见[验收记录末节](../reports/ISSUE6_PARADIGM_ACCEPTANCE.md)、[执行报告](../reports/ISSUE6_PARADIGM_IMPLEMENTATION_REPORT.md)和[工程合同](../docs/ISSUE6_PARADIGM_CONTRACT.md)。前次 231 passed 与暂不通过描述为历史；旧[草案](../docs/tasks/ISSUE6_PARADIGM_EXTENSION_DRAFT.md)仅作历史。
- Issue 5+6 实现、测试、合同与事实报告纳入本次发布；本地测试留下的 `:memory:.ses` 不纳入版本控制。无生产范式包或批准的 A/B/C 参数；Windows/真实 Rally/物理输出验收不由合成证据替代。

### 历史状态（后续结论优先）

P4-B 最终工程验收通过（2026-09-21）：自然EOF停止登记、事件排空与writer关闭屏障已复核，详见[验收记录末节](../reports/P4B_RALLY_START_STOP_ACCEPTANCE.md)。执行者完整203 passed、专项54 passed；主线程未重跑，实际diff --check通过。此前暂不通过/待收口描述为历史，已由此结果替代。未提交/推送；Windows原生、真实Rally及物理输出仍待独立验证。

P4-B 第二轮回报201 passed；主线程复核年龄/容量修改，仍发现自然EOF先放行writer后申请停止租约的竞态及最终事件排空问题，暂未整轮验收。已发布[记录收尾补充任务](../docs/tasks/P4B_RALLY_RECORDING_CLOSEOUT.md)，证据见验收记录末节。

P4-B 已回报193 passed，主线程[复核](../reports/P4B_RALLY_START_STOP_ACCEPTANCE.md)暂不通过：持续年龄保护缺失、记录收尾可阻止保护性停止、隔离资源上限未预留停止额度。已发布[收口任务](../docs/tasks/P4B_RALLY_START_STOP_CLOSEOUT.md)，待执行。未提交/推送、未真机。

2026-09-21 主线程更新：正式本地基线为 `/Users/xuqinghe/sleep/main-development`，main@2a07e4f3386692c41ee4176b48babd700bb07fe9；基线验收通过，执行者完整回归 184 passed，主线程未重复运行。已发布 [P4-B 实现 prompt](../docs/tasks/P4B_RALLY_START_STOP_EXECUTION.md)，尚未实现；本轮只授权工程与假端合成验证，不授权真机刺激、提交或推送。本文下方较早版本描述与最新实现有差异时，以当前代码和本 prompt 为准：默认 NoModel；Curry 网络短包由控制端累积为 30 秒窗口。

**最新目标修正：正式部署平台为 Windows；单通道 ONNX 睡眠分期已接入；下一项为睡眠分期与 Rally API 启停联动。** 预处理冻结为输入 µV、50 Hz 工频处理、0.3–35 Hz 带通、100 Hz、不做逐样本 z-score；模型时间长度从 ONNX 自动读取并要求为 3000 的整数倍，通道名由用户选择且完整唯一匹配。默认 LiteSleepNet EDF20 FP32 6000 点模型已复制进控制器发布包，LiteSleepNet 原仓库保持只读。Windows 锁定安装、原生 GUI、当前工作区中文/空格路径、TCP/UDP 合成回环和真实 ONNX GUI 回环均已于 2026-09-16 验证通过。2026-09-19 用户在远程实验电脑验证 Rally 的 `Start Stim` / `Stop Stim` API 可以实际启动、停止当前已加载的刺激协议；回复来自本机动态 UDP 端口。下一工程优先项是按下述 P4-B 合同实现真实联动；真实模型效果、刺激参数/范式、时钟同步与受试者实验仍未验证或授权。

开发睡眠分期实验电刺激控制器：Curry 8 每包 30 秒 EEG，预留模型接口进行 W/N1/N2/N3/REM 分期，用户多选目标睡眠期控制刺激。当前资料阅读、归档、需求整理和开发目录规范化已完成，开发流程规划已形成；P1 收口回报已复核，工程范围通过主线程验收。执行者最终回归 35 passed；主线程核对关键实现、测试与截图，未重复跑全套测试。P2 收口修复已复核，工程范围通过主线程验收（执行者最终回归 52 passed）。实时/回放隔离问题已收口；原生 macOS 启动 139 的当前 shell 失败路径已在 P4-A 定位至零屏幕环境下的 Qt 屏幕查询；零屏幕成因及普通桌面可用性仍待验证。P3 实现回报已复核，模拟工程范围通过主线程验收；执行者完整回归 88 passed，主线程核对关键代码、测试与默认界面截图，未重跑测试。P4-A 诊断与接入文档已通过主线程验收；系统崩溃栈已核对，原生桌面启动/布局/关闭仍待验证，139 未判为已修复。P-GUI 已通过工程验收，执行者最终回归 89 passed；原生本轮退出 134、零屏幕问题仍未解决。真机与同步后续批次未发布。

## 已完成与入口

- [ONNX 睡眠分期接入](../reports/2026-09-16_ONNX_STAGING_INTEGRATION.md)：默认 6000 点模型、可选单通道、状态化滤波、自动时间维合同、W/N1/N2/N3/REM 输出和 GUI 回环已完成；最终完整回归 97 passed，wheel 包含模型与身份清单。详细语义见 [ONNX 合同](../docs/P_MODEL_ONNX_CONTRACT.md)。

- [Windows 安装、原生 GUI、路径权限与回环验证](../reports/2026-09-16_WINDOWS_VERIFICATION.md)：CPython 3.11.15 / PySide6 6.11.2 锁定安装成功，原生 Qt `windows` 平台窗口可见并正常关闭，中文路径完整会话读回通过，完整回归 89 passed，三个关键 TCP/UDP 回环用例复跑通过；未连接真实设备。Windows 截图工具受 `0x80004002` 限制，本轮无原生截图。

- [用户需求与范围](../docs/SLEEP_STIM_REQUIREMENTS.md)：模型本体不在开发范围，UI 模板只作基础；用户最新说明优先于根规则中的旧项目设备背景。
- [参考资料索引](../reference/README.md)：电刺激原件、UI 原包及展开文件、Curry 克隆。
- [静态分析记录](../reports/2026-09-15_reference_review.md)：Rally 接口语义、版本差异、旧脚本问题与同步证据边界。
- Curry 仓库已克隆至 `curry8-netstream/`，main @ `0d1067c`；现有 uv `.venv` 已配齐 Curry/UI 开发依赖，导入和离屏 UI 验证已通过。

- [开发目录整理记录](../reports/2026-09-15_development_layout.md)：根级 src 包、pyproject.toml、uv.lock 与统一测试入口已建立；uv sync --locked 成功，11 项 Curry 既有测试本轮实际通过。根目录仍未初始化 Git。

## 当前开发计划

- [开发流程](plans/SLEEP_STIM_DEVELOPMENT_PLAN.md)：P0 最小合同 → P1 桌面与 EEG → P2 分期接口/记录 → P3 刺激策略与模拟联调 → P4 真机与同步 → P5 收口。计划已整理，详细接口和实验参数尚未冻结。

## 当前执行任务：P4-B ONNX 分期—Rally API 启停联动

本任务把**实时且有效的 ONNX 睡眠分期结果**转换为对 Rally 当前已加载刺激协议的二值启停控制。目标睡眠期触发 `Start Stim`，非目标睡眠期触发 `Stop Stim`。不使用低电流、低频率或其他“静默协议”；本批不选择、切换或修改具体刺激范式，实验人员须先在 Rally 中加载并检查协议。

供执行人员使用的自包含任务快照已放入 [reference/CURRENT_TASK_QUEUE.md](../reference/CURRENT_TASK_QUEUE.md)。本文件仍是动态权威队列；`reference` 文件是本批交付副本，不应由执行人员反向改写为新的并行状态源。

### 已确认接口证据

- Rally UDP 请求端点：`127.0.0.1:8801`；UTF-8 命令为 `Start Stim` 和 `Stop Stim`。
- 启动成功回复：`启动刺激成功`；停止成功回复：`停止刺激成功`。现有兼容回复 `RALLY_ERROR_SUCCESS` 可保留，但不得把任意非空回复判为成功。
- Rally 回复可能来自 `127.0.0.1` 的动态 UDP 源端口，不要求回复源端口等于 `8801`；必须校验回复主机为本机、记录实际源端口并设置有限超时。
- 2026-09-19 的人工测试已观察到 API 启动和停止均能控制设备。本证据只证明工程命令链路，不证明刺激参数、安全性、生理输出、模型效果或正式实验有效性。
- 实现参考为 `reference/RallyStartStopTest.py`、`reference/START_STOP_TESTING.md` 及原始 `reference/RealtimeControl*.py`。参考文件中的固定实验时间线或参数不自动成为本项目合同。

### 冻结行为合同

1. **分期结果资格**：仅当前实时会话、当前连接、`ProcessingStatus.SUCCESS`、合法标签 `W/N1/N2/N3/REM`、新鲜且块序单调递增的 ONNX 结果可作为正常睡眠期决策依据。无效标签、失败、取消、不可用、过期或其他不合格结果不能触发 `Start Stim`，也不能被当作新的正常目标期/非目标期结果；当前会话的合同既定故障另按第 8 条保护性收尾。回放、离线 NPY、历史结果以及其他 session/generation 的结果不得改变当前控制状态或触发任何控制命令。
2. **目标期设置与默认隔离**：继续使用 GUI 中显式选择的目标睡眠期；默认目标集合为空，真实 Rally 自动控制默认关闭。默认未启用真实控制的场景（包括默认 `NoModel` 和回放），不论收到何种分期结果，均为零真实请求。显式启用后的基线 `Stop Stim`、用户关闭自动控制/会话退出时的保护性 `Stop Stim`、以及启动结果不确定后的单次补偿 `Stop Stim` 是独立控制动作，不受分期资格过滤的零请求规则禁止。目标期映射、持续时间、确认 epoch 数、置信度阈值与具体刺激协议属于后续范式任务，本批不得擅自冻结。
3. **二值决策**：有效标签在目标集合内时期望状态为运行；不在目标集合内时期望状态为停止。重复的同向判定不得重复发送命令：已确认运行时重复目标期为 no-op，已确认停止时重复非目标期为 no-op。
4. **明确状态机**：至少区分 `DISARMED/UNKNOWN`、`STOPPED`、`STARTING`、`RUNNING`、`STOPPING`、`FAULT/UNKNOWN`；界面只能显示最后一次被 Rally 明确确认的状态，不得把“UDP 已发送”显示成“设备已启动/已停止”。
5. **启用基线**：用户每次显式启用真实自动控制时，先发送一次 `Stop Stim` 建立安全基线；只有收到精确停止成功回复后才进入 `STOPPED` 并允许后续分期触发启动。基线停止失败或超时则不允许自动启动。
6. **单请求与停止优先**：同一时刻只允许一个 Rally 请求在途；阶段快速变化不得形成无界命令队列。请求完成后基于最新合格结果重新计算期望状态，停止、断开和退出路径优先于启动。
7. **启动不确定处理**：`Start Stim` 拒绝、未知回复或超时时，不重试启动；立即做一次有界补偿 `Stop Stim`，关闭本次真实自动控制并将状态标为未知，直到停止得到明确确认。不得循环重试。
8. **故障停止路径**：仅当前实时 session/generation 中、真实控制已启用的会话故障（包括模型失败、Curry 断流/断开、块序中断，或运行中切换到不可用/`NoModel`），以及该控制会话中用户关闭自动控制/断开会话/正常退出应用，才走此收尾路径。设备未被确认停止时，执行一次有界保护性 `Stop Stim`；该停止是故障/关闭收尾，不是由无效标签推导出的睡眠期刺激决策。设备已确认停止时不重复发送。停止未获明确回复时显示 `FAULT/UNKNOWN`，提示操作者在 Rally/硬件侧独立停止；不得伪造“已停止”。回放、离线、历史或其他 session/generation 的迟到结果（包括错误）不得改变当前控制状态，也不得对当前或新会话触发 `Start Stim` / `Stop Stim`。
9. **真机与模拟隔离**：真实 Rally 控制与现有回环模拟器必须是互斥模式。真实模式仅允许固定本机端点 `127.0.0.1:8801`，首批不开放任意远程 IP；回放永远只能展示记录，不能重发控制命令。
10. **人工控制权**：真实控制每个应用会话都要显式确认并启用，界面提供醒目的模式、期望状态、最后确认状态、最近命令/回复及故障提示。软件按钮不是硬件急停；Rally/硬件独立停止方式必须保留并写入运行说明。
11. **会话记录**：通过现有 session 单写者追加控制事件，不改写原始 EEG。至少记录控制模式、事件类型、分期标签、块号、模型标识、目标集合、命令、发送/接收时间、回复原文、回复主机与动态端口、命令前后状态及结果类别。回放仅展示这些事件。
12. **本批不做**：不调用或改写 `RealtimeControlAPIDemo1...5.py` 来切换协议，不移植 `RealtimeControlManager.py` 的固定时间线，不设计 N2/N3 对应哪套范式，不执行正式受试者实验，不把工程启停成功解释为安全性或科研效果。

### 实施与验证要求

- 在现有刺激传输层和策略入口上增量实现，不另建第二套分期、会话或记录框架；P3 模拟合同保留为历史和回归基线，本任务仅对明确启用的真实模式放开固定 Rally 端点。
- 单元测试覆盖：结果资格过滤、目标/非目标映射、状态转换、幂等、单请求、停止优先、断流/模型失败/关闭路径，以及启动不确定后的单次补偿停止。明确断言不合格结果不能触发 `Start Stim` 或形成新的正常睡眠期决策；当前启用会话的既定故障在设备未确认停止时走一次保护性停止。
- UDP 假端测试覆盖：动态回复端口、两条精确中文成功回复、兼容成功回复、拒绝、未知回复、超时、迟到回复和非本机来源；不得用真实设备完成自动化测试。
- 集成测试覆盖：合成 Curry 数据 → 真实 ONNX 或受控分期结果 → 假 Rally → GUI 状态与 session 事件，并分别断言：默认未启用真实控制 + 无效结果为零请求；当前已启用实时会话发生模型失败且停止未确认时恰有一次保护性 `Stop Stim`、没有 `Start Stim`；旧 session/generation 的失败/取消/过期迟到结果不改变新会话状态且不发命令；回放不发命令。显式启用时的基线停止、用户关闭/退出停止和启动不确定补偿停止应按各自场景验证，不计入“分期不合格结果零请求”断言。
- 完成自动化测试后，才可由实验人员在无受试者、已检查刺激协议且硬件独立停止可用的条件下进行一次限界真机验收：基线停止 → 目标期启动 → 重复目标期不重复启动 → 非目标期停止 → 故障/关闭停止。逐步核对 Rally 界面和物理输出；任何不一致立即使用独立停止方式终止。
- 执行人员应提交实现、相关测试、运行说明和事实报告；报告区分“代码/测试通过”“Rally 回复确认”“物理输出确认”三种证据，不得合并表述。最终运行完整回归并保留已有功能。

### 当前授权边界

用户已授权把本任务写入队列并交由对应人员实施。此授权不等于允许本线程发送真实刺激命令，也不包含刺激范式选择、协议参数修改、受试者实验、正式采集次数或科研结论。上述事项后续单独安排。

## 最新完成任务：P-GUI

用户要求取消控制器中的 EEG 波形显示，由 Curry 8 查看；本轮重新组织控制界面。

- [P-GUI 合同](../docs/P_GUI_CONTRACT.md)：替代 P1/P2 波形展示要求，保留 EEG 接收、分期、记录与回放数据链路。
- [P-GUI 执行 prompt](../docs/tasks/P_GUI_EXECUTION.md)：已实施并通过主线程工程验收。
- [P-GUI 执行报告](../reports/P_GUI_REPORT.md)：去除波形/PyQtGraph，执行者最终回归 89 passed，离屏启动关闭通过。
- [P-GUI 主线程验收](../reports/P_GUI_ACCEPTANCE.md)：代码、测试与四张截图已复核，主线程未重跑测试。原生本轮退出 134，零屏幕限制仍在。

## P4-A 文档与诊断已验收，原生验证待完成

- [P4 接入准备合同](../docs/P4_INTEGRATION_READINESS_CONTRACT.md)：先处理原生启动 139 与接入资料，真实采集/刺激/同步另行定界。
- [P4-A 执行 prompt](../docs/tasks/P4A_NATIVE_READINESS_EXECUTION.md)：可独立交付部分已完成；原生窗口验证受执行环境限制。
- [P4-A 执行报告](../reports/P4A_NATIVE_READINESS_REPORT.md)：Cocoa 零屏幕，最小窗口 134、正式入口 139；未改源码/依赖、未重跑测试。
- [P4 接入手册](../docs/P4_INTEGRATION_RUNBOOK.md)：正常桌面核验步骤及后续资料缺项。
- [P4-A 主线程验收](../reports/P4A_NATIVE_READINESS_ACCEPTANCE.md)：诊断/文档通过；待正常桌面提供窗口、800×600 与关闭证据。

## P3 已完成证据

- [P3 合同](../docs/P3_STIMULATION_SIMULATION_CONTRACT.md)：目标期多选、显式策略、Rally 报文与受控本机模拟联调；不启用真实刺激。
- [P3 执行 prompt](../docs/tasks/P3_STIMULATION_SIMULATION_EXECUTION.md)：已实施，模拟工程范围验收通过。
- [P3 执行报告](../reports/P3_STIMULATION_SIMULATION_REPORT.md)：88 passed，默认 NoModel 零请求，仅本应用回环模拟端联调。
- [P3 主线程验收](../reports/P3_STIMULATION_SIMULATION_ACCEPTANCE.md)：策略、端点约束、迟到回复隔离、单写者收尾与回放隔离已复核；原生 GUI 与真机限制保留。

### P2 已完成证据

- [P2 控制性合同](../docs/P2_STAGING_RECORDING_CONTRACT.md)：分期接口、保存默认关闭、v1 会话记录与只读回放；详细实验参数不在本阶段。
- [P2 执行 prompt](../docs/tasks/P2_STAGING_RECORDING_EXECUTION.md)：已实施并完成末尾收口要求，工程验收通过。
- [P2 执行报告](../reports/P2_STAGING_RECORDING_REPORT.md)：最终 52 passed，保留原 49 passed；原生启动退出 139，根因未确定。
- [P2 主线程验收](../reports/P2_STAGING_RECORDING_ACCEPTANCE.md)：末节记录工程验收通过，实时/回放隔离已修复；保留原生可用性待验证项。

### P1 已完成证据

- [P1 桌面与 EEG 接入执行 prompt](../docs/tasks/P1_DESKTOP_EEG_EXECUTION.md)：已实施、补齐并通过主线程验收；文末补充要求保留为历史。
- [P1 控制性合同](../docs/P1_DESKTOP_EEG_CONTRACT.md)：P1 接口、状态、显示和验证已定；不代表整个 P0 的模型/刺激合同已完成。
- [执行报告](../reports/P1_DESKTOP_EEG_REPORT.md)：最终 35 passed，保留首版 24 passed；离屏启动/关闭与最小窗口验证完成，未真机、未原生窗口验收。
- [主线程验收记录](../reports/P1_DESKTOP_EEG_ACCEPTANCE.md)：终态历史标注已修复，半包取消/带连接关闭等证据已补齐；末节记录工程验收通过。主线程未重跑全套测试。

## 授权与后续

用户允许后续整合并优化电刺激代码。P1/P2/P3 与 Windows 原生验证已经完成；本轮单通道 ONNX 睡眠分期也已实现并通过合成回环。用户已在远程实验电脑完成一次 Rally API 人工启停验证，P4-B 实现任务现已发布到本队列与 `reference` 执行快照；控制器中的自动联动尚未实现。真实模型效果、刺激范式、时钟同步及正式实验仍未实施。

Rally 的本机 API 启停端点与成功回复已取得真机工程证据；目标 Curry 的正式通道/参考配置、实际刺激参数、睡眠期到范式的映射、时序确认规则及同步合同仍未冻结。杀掉 Python 进程不等于设备停止，软件停止失败时必须使用 Rally/硬件独立停止方式；详见 P4-B 合同和分析记录。时钟同步保持后续实施边界，不因参考代码到位自动启动。

## GitHub 发布记录

按用户授权已推送至 `https://github.com/GTOING/curry8-netstream` 的 main，提交 `c68db418dbfada7a489574ca1e88fbaceb25fdd1`，GitHub API 已核对远端 SHA。控制器位于 `sleep-stim-controller/`，包含当前 Curry 依赖源码及本地适配；保留原仓库根内容和历史。未上传虚拟环境、采集数据或 reference 原件。发布工作克隆位于 `/private/tmp/sleep-github-publish-20260916`；当前开发根目录仍未初始化 Git，原嵌套 Curry 工作区改动保留。此发布记录在推送完成后本地追加，尚未包含在上述提交中。

## GitHub 根结构更正

用户要求按当前本地项目直接作为远端根目录发布，替代之前 `sleep-stim-controller/` 包裹结构；不保留旧远端根文件布局，保留提交历史。当前 Curry 适配源码作为项目依赖一并发布，嵌套 Git 元数据、环境、缓存、数据和被忽略参考原件不上传。本次发布执行结果以对话中核实的远端提交为准。
